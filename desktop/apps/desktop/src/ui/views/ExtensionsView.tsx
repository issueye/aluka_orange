 ...
 * 扩展与技能视图（「扩展」页面）
 *
 * 自包含：挂载即加载已装插件 / 已加载扩展 / 技能清单；
 * 「发现」Tab 负责 pi 生态市场查询（320ms 防抖 + 触底无限滚动）；
 * 安装结果经 package.install 事件回传，本视图自行监听并同步目录状态。
 */
import { useEffect, useMemo, useRef, useState } from "react";
import { rpc, bridge } from "../bridge.ts";
import { Button, SectionHead } from "../components/index.ts";
import type { InstalledPkg, MarketRow, Toast } from "../types.ts";
...
export function ExtensionsView(props: {
  ...
  // ── 市场目录（发现 Tab） ──
  const [tab, setTab] = useState<"installed" | "discover">("installed"); // 扩展页子 Tab
  ...
  async function refreshExtensions() {
    const [inv, skillList] = await Promise.all([
      ...
      rpc<Array<{ name: string; description: string; path: string }>>("listSkills"),
    ]);
  ...
  async function removeMarket(name: string) {
    ...
    try {
      ...
      if (result?.ok) {
        ...
        await Promise.all([loadInstalledPackages(), props.onSettingsChanged(), refreshExtensions()]);
      } else {
  ...
  useEffect(() => {
    ...
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [tab, catalog.length, catalogTotal, catalogLoading, catalogLoadingMore, catalogQuery]);
  ...
  return (
    <div className="settings-page" data-aluka-drag="no-drag">
      <div className="settings-page-inner">
        <SectionHead
          title="扩展与技能"
          hint="「发现」搜索 pi 生态插件市场（pi.dev/packages 数据源：npm keyword pi-package），触底自动加载更多；「已安装」展示插件、当前工作目录加载的扩展、错误与技能。"
        />
        <div className="ext-head-actions">
          <span className="hint">{extSummary}</span>
